########################################################################
### Functions for simulations of population dynamics for finite area ###
########################################################################
	  ...
	  G[i,,] <- plogis(
		...
		for(t in 1:nt){
		  ...
		  #   )
			...
			ng[i,t,] <- rbinom(nj,prob=G[i,t,],size=ns[i,t,])

			### OLD SEED SURVIVAL ###

			no[i,t,] <- rbinom(nj,prob=So[i,t,],size=ns[i,t,]-ng[i,t,])
			...
			for(j in 1:nj){
				...
				if(ng[i,t,j]>0){
					...
					eta_bar_t[,j] <- beta_r[i,j,] %*% t(x_t)
						# previously  t(x_t[,-3]); -3 removes ^2 term
						# [species,sites]

					pi_t[,j] <- pi_bar_t[,j] + eps_y_p[i,t,j] + eps_s_p[i,,j] + eps_o_p_t[,j]
					eta_t[,j] <- eta_bar_t[,j] + eps_y_r[i,t,j] + eps_s_r[i,,j]

					nr_t[,j] <- rbinom(nk,prob=plogis(pi_t[,j]),size=ng_t[,j])
						# number of seedlings in each plot that reproduced
					...
					if(sum(nr_t[,j])>0){	
						nzpos <- which(nr_t[,j]>0)
						mus <- rep(exp(eta_t[nzpos,j]),nr_t[nzpos,j])
			...
			Sn[i,t,] <- BHS(nn[i,t,]/tottarea,m0[i,t,],m1[i,t,])

			nnb[i,t,] <- rbinom(nj,prob=Sn[i,t,],size=nn[i,t,])
