      ...
      for(j in 1:nj){
        ...
        if(ng[i,t,j] >= ngmin){
          ...
          fnn <- function(x){
            ...
            dg <- dnorm(lg,mean=lgmu,sd=sig_s_g[i,j])
            ...
            pr_t <- plogis(lp) * dnorm(lp, 
              mean = pi_bar_t + eps_y_p[i,t,j], 
              sd = sqrt(sig_s_p[i]^2 + sig_o_p[i]^2)
              )
            # code borrowed from logitnorm package
            
            rs_t <- nbtmean(mu=exp(lr),phi[i]) * dnorm(lr, 
              mean = eta_bar_t + eps_y_r[i,t,j], 
              sd = sig_s_r[i]
              )
            ...
            return(exp(log(dg) + lnY_t)) 
            # expected overall mean density of seeds
          } # close function to be integrated
          
          # sig_s_g[i,j]
          # sqrt(sig_s_p[i]^2 + sig_o_p[i]^2)
          # sig_s_r[i]
          # !!! sigma_pt <- eta_t - intsd * sigma_t,!!!
          ...
          nn[i,t,j] <- hcubature(f=fnn, 
            lowerLimit = c(glo, plo, ylo),
            upperLimit = c(ghi, phi, yhi),
