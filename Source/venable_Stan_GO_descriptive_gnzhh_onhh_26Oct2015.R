...
source("venable_figure_functions_08Aug2015.R")

csyp <- read.csv("csyp_03Oct2015.csv",header=T)
ssyp <- read.csv("ssyp_03Oct2015.csv",header=T)
msy <- read.csv("msy_03Oct2015.csv",header=T)
...
odata <- subset(ssyp,is.na(totlive)==F,select=c("species","year","plot","totlive","area_o"))
...
ofit_nhh <- stan(model_code=omod_nhh,data=dat_list,chains=0)
...
go_sflist <- parLapply(CL, 1:nchains, function(i){
  require(rstan)
  read_stan_csv(paste0(mod_names[fit_ind[i]],"_chain",chain_ind[i],"_25Oct2015.csv"))
  })
