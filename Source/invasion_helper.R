...
evolve <- function(
  ...
  z <- params$zsdo * (series$zn*params$msd + params$zamo+params$mam)
  w <- params$wsdo * (series$wn*params$msd + params$wamo+params$mam)
    # check these to make sure that correlation has remained the same

  eps_y_p <- params$pn * sig_y_p
  eps_y_r <- params$rn * sig_y_r
  ...
  with(params,{
    ...
    for(r in 1:nr){
      ...
      if(nk %in% c(0,Inf)){
        rd <- with(es[r,], ressim(wt,x_z,am,bm,# as,bs,abr,
                                  beta_p,beta_r,
                                  eps_y_p,eps_y_r,
                                  sig_s_g,sig_s_p,sig_s_r,
                                  sig_o_p,phi_r,theta_g,
        ...
        invaded <- invade_infinite(wt,ami,bmi,rd$Gres,rd$Ye,So,nt,nb) 
        # asi,bsi,abri,
      }
      ...
      if(nk>0 & nk<Inf){
        ...
        invaded <- with(es[r,], invade_finite(w=wt,x_z,am,bm,ami,bmi,
                                              beta_p,beta_r,
                                              eps_y_p,eps_y_r,
                                              eps_s_g,eps_s_p,eps_s_r,
                                              sig_o_p,phi_r,
  ...
  # ADD TIME OF EXTINCTION TO ES
  
  outlist <- list(zw=zw,es=es) # do we need "with" here?
